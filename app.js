...
const PET_STAGES = [
    { level: 1, avatar: '🥚', msg: 'Waiting to hatch...' },
    { level: 3, avatar: '🐣', msg: 'I just hatched!' },
    { level: 5, avatar: '🐥', msg: 'Growing strong!' },
    { level: 10, avatar: '🦉', msg: 'Wise Guardian' },
    { level: 20, avatar: '🐲', msg: 'Legendary Dragon' }
];
...
function renderHistory() {
    ...
    if (weeklyTasks.length === 0) {
        historyList.innerHTML = '<p style="text-align:center; color:var(--text-secondary)">No quests completed this week yet.</p>';
        return;
    }
...
function toggleTask(id) {
    ...
    if (task) {
        ...
        // Delay for animation then remove
        setTimeout(() => {
            if (li) {
            ...
            setTimeout(() => {
                ...
                saveTasks(); // This re-renders and removes it from the list
    ...
    if (memos.length === 0) {
        ...
        memos.forEach(memo => {
            ...
            div.innerHTML = `
                ...
                <button class="memo-delete-btn" onclick="deleteMemo(${memo.id})">
